...
for {set i 0} {$i < [expr $NSwitches - 4]} {incr i} {
  $ns duplex-link $sw($i) $sw([expr $i+4]) $link_cap $link_delay $queue_type
}
...
for {set i 0} {$i < [expr $MPTCP_Core_Nodes-4] } {incr i} {
    ...
    for {set j 0} {$j < $MPTCP_subflows} {incr j} {
      ...
      $ns duplex-link $n_([expr $i+4]-$j) $sw([expr $j+4]) $link_cap $link_delay $queue_type
    }   
}
...
for {set i 0} {$i < $NTCP} {incr i} {
    ...
    $ctcp($i) set dctcp_ false
    $ns attach-agent $c($i) $ctcp($i)
    

    if {$i == 1} {
      $ns duplex-link $c([expr $i-1]) $sw([expr $i-1]) $link_cap $link_delay $queue_type
      $ns duplex-link $c($i) $sw([expr $i+3]) $link_cap $link_delay $queue_type

      set ftpc([expr $i-1]) [new Application/FTP]
