...
for {set i 0} {$i < $NSwitches} {incr i} {
  ...
  if {$i == 0} {
    ...
    $ns duplex-link $c([expr $i-2]) $sw($i) $link_cap $link_delay $queue_type
...
proc myTrace {file} {
  global ns N samp_int tcp0 tcp1 mtrace MPTCP_Core_Nodes MPTCP_subflows tcp0 tcp1 ctcp0
  ...
  for {set i 0} {$i < [expr $MPTCP_Core_Nodes-1]} {incr i} {
    ...
    for {set j 0} {$j < $MPTCP_subflows} {incr j} {
      ...
      if {$j == 0} {
        ...
        puts -nonewline $file "$now [$ctcp0 set cwnd_]"
        # puts $file "$now [$ctcp0 set cwnd_]"
