...
# checking if the destination (column 3) corresponds to node in arg 1
            if (($x[3] eq $destnode1 && $x[8] eq $fromport1 && $x[9] eq $toport1) || ($x[3] eq $destnode2 && $x[8] eq $fromport2 && $x[9] eq $toport2) ) 
            { 
...
# One interval has passed, compute throughput
   ...
   print STDERR "Avg throughput $fromport - $toport = ",8*$mib*$grantsum/$clock,"MBytes/sec \n";  
   close DATA;
   exit(0);
}
